...
| --- | --- | --- | --- |
| Claude | `claude-opus-4-8` | `max` | `3` USD default cap |
...
Do not silently downgrade either reviewer. If Claude Opus 4.8, GPT-5.5, or the requested reasoning effort is unavailable, report the limitation and ask before using a fallback.
...
claude_effort="${CLAUDE_GPT_REVIEW_CLAUDE_EFFORT:-max}"
...
- Prefer the default `claude-opus-4-8 --effort max` and `gpt-5.5` with `model_reasoning_effort="xhigh"` settings.
